...
class ZSectionFormed(GeneralSection):
    ...
    def __init__(self, hw: float, wlf: float, wuf: float, ts: float,
        ...
        self.ts = ts
        self.rm = rm
        y = [0.0, self.wlf, self.wlf, self.ts, self.ts,
             self.ts-self.wuf, self.ts-self.wuf, 0.0]
        z = [0.0, 0.0, self.ts, self.ts, self.hw, self.hw, self.hw-self.ts,
             self.hw-self.ts]
        r = [self.rm+self.ts/2, 0.0, 0.0, self.rm-self.ts/2, self.rm+self.ts/2,
             0.0, 0.0, self.rm-self.ts/2]
        super().__init__(y, z, r, label=label)

    def to_thin_walled_section(self) -> ThinWalledSection:
        y = [self.wlf, self.ts/2, self.ts/2, self.ts/2-self.wuf]
